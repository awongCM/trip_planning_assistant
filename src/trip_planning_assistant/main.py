...
def train():
    ...
    try:
        TripPlanningAssistant().crew().train(n_iterations=int(sys.argv[1]), filename=sys.argv[2], inputs=inputs)
...
def test():
    ...
    try:
        TripPlanningAssistant().crew().test(n_iterations=int(sys.argv[1]), eval_llm=sys.argv[2], inputs=inputs)
